const createPostItem = (post, viewedPosts, i18nextInstance) => {
  ...
  item.classList.add('d-flex', 'list-group-item', 'justify-content-between', 'align-items-start');
  const link = document.createElement('a');
  const linkFontWeight = viewedPosts.has(post.id) ? 'fw-normal' : 'fw-bold';
...
const renderPosts = (state, elements, i18nextInstance) => {
  ...
  const postItems = state.posts.map(
    (post) => createPostItem(post, state.uiState.viewedPostsIds, i18nextInstance),
  );
